 ...
 */
module.exports.buildGeminiPrompt = function(
  reportTitle, 
  transcription, 
  issuesForPrompt, 
  qualityCodesJson, 
  ...
  return `
    ...
    Maintain the exact relationship between issues and their 'image_filenames'. 
    Also, preserve the original 'id' for each issue.

    CONTEXT 1: REPORT TITLE:
    ${reportTitle}
    ---
    CONTEXT 2: AUDIO TRANSCRIPT:
    ${transcription}
    ---
    CONTEXT 3: ISSUES AND IMAGES JSON:
    ${JSON.stringify(issuesForPrompt, null, 2)}
    ---
    CONTEXT 4: QUALITY CODES LIBRARY:
    ${JSON.stringify(qualityCodesJson, null, 2)}
    ---
    CONTEXT 5: JSON REPORT TEMPLATE (YOUR OUTPUT MUST MATCH THIS STRUCTURE):
